...
import '../logic/strategy_inspector.dart';
...
class GameProvider with ChangeNotifier {
  ...
  Future<void> initGame() async {
    ...
    // [수정된 부분]
    // train -> runTrainingInBackground로 변경
    // (progress) -> (logMessage)로 변경 (이제 숫자가 아니라 로그 문자열이 옴)
    await _solver.runTrainingInBackground(trainingIterations, (logMessage) {
      // 진행 상황을 안드로이드 스튜디오 콘솔에 출력 (앱이 멈추지 않음 확인용)
      debugPrint(logMessage);
    });

    log("✅ Training Complete!", "sys");
    var inspector = StrategyInspector(_solver);
    inspector.printKeyStrategies();
  ...
  void addLog(String message) {
    log(message, "sys"); // 기본 타입은 'sys'로 처리
  }
}
